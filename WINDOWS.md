...
The console-free forwarder update was deployed after the clone finished, and its SOCKS handshake passed. The installed `windows_acceptance.ps1 -ListenerOnly` rerun was blocked by `LocalMachine: Restricted` with no Group Policy override; the file had no internet-zone marker. No execution policy was changed and no indirect script execution was used. Separate approval for a process-only policy remains pending; the earlier real IPv4/IPv6 listener evidence is not relabelled as a post-update script run.
