...
The user subsequently authenticated HTTPS Git interactively. The helper's read-only precheck passed before starting a full clone into the requested NTFS directory. The exact-URL `socks5h` setting was verified in that repository's local configuration, and the live Git HTTPS process had one established socket to `127.0.0.1:1080` with no direct established sockets. Clone completion remains unverified while the transfer runs; no push was performed. The user explicitly confirmed that the supplied task-tracker page opens in the dedicated Chrome profile. This is a manual positive browser check, not an automated visual result. Browser failure with the gateway off remains untested; disruptive checks and forwarder updates are deferred while Git transfers data.
...
Earlier Python-generated host-preservation evidence covered IPv4 only. CLI snapshots now separately collect IPv6 DNS, non-local routes and default routes, and Windows validation requires their successful comparison. Missing legacy IPv6 data, failed cmdlets or malformed JSON cannot pass. The standalone PowerShell acceptance script already fingerprints both address families. Real Windows IPv6 collection passed and the selected IPv6 route used the configured outer interface; public IPv6 egress was not tested. A new private observation was saved during the live clone without replacing the old preconnection baseline. IPv6 preservation across a controlled VPN lifecycle transition remains pending; no prior unmeasured result is inferred. None of these diagnostics changes network settings.

A Docker CLI (29.7.2, context `desktop-linux`) is now present, but the Docker Linux engine is not running. Actual Docker-backend, full corporate-clone completion, browser failure with the gateway off, TCP transport, remote LAN-device and full uninstall acceptance remain incomplete. Existing macOS behavior is tested with mocks on Windows, not an actual macOS machine. The automated suite runs 105 tests: 103 pass and 2 platform-specific cases are skipped on Windows. It includes actual Windows IPv4/IPv6 TCP probe controls, a console-free binary bridge check, locale-independent PowerShell JSON output and IPv6 network queries; these Windows-only cases are skipped on macOS/Linux. Automated tests cannot substitute for missing network checks. Diagnostic JSON and private deployment inputs remain outside Git.
