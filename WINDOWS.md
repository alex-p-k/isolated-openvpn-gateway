...
After all Docker processes exited, the owner approved renaming only this socket to a same-directory backup, with no overwrite. Both PowerShell `Rename-Item` and native `MoveFileExW` with flags zero returned 1920. A metadata-only `CreateFileW` probe using `OPEN_EXISTING` and `FILE_FLAG_OPEN_REPARSE_POINT` also failed. The original remains, no backup was created, and Docker was not relaunched after the failed rename. Nothing was deleted; no ACL, driver, service, registry, firewall or WSL configuration was changed. No delayed-on-reboot operation was scheduled. A factory reset, reparse-tag removal or replacement of Docker directories is outside this approved repair and must not be automated as a fallback.

The existing UDP WSL gateway remained ready, with only `127.0.0.1` listening on the configured SOCKS port. Post-attempt checks matched the pre-Docker baseline for Windows IPv4/IPv6 DNS/routes, ordinary public IPv4 egress and global Git proxy settings. The Docker backend remains blocked until Desktop can start; successful WSL tests do not establish Docker acceptance. Keep raw Desktop logs private and do not upload a diagnostic bundle automatically. The attempted repair has no file changes to roll back.
