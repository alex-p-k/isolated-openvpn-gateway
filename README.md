...
The console-free WSL forwarder update was applied after the clone finished. A fresh UDP1194 positive/negative test then passed, but the later multi-transport attempt did not restore the session; final checks confirmed OpenVPN stopped, credentials removed and no SOCKS listener. Credential-free Windows/root-WSL/nested-namespace egress checks subsequently matched again, with host IPv4/IPv6 DNS/routes preserved. Do not infer current connectivity from an earlier successful test. Browser failure with the gateway off, TCP transport, remote LAN-device and full uninstall acceptance remain incomplete. The installed PowerShell listener-script rerun is also pending separate approval of a process-only execution policy; the machine's `Restricted` policy was not changed. A Docker CLI is present, but its Linux engine is stopped; no real Docker-backend acceptance was performed. macOS compatibility is covered by tests/mocks, not a real macOS run. See `WINDOWS.md` for the evidence boundaries.
