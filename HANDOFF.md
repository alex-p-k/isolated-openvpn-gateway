...
Владелец тестовой машины также подтвердил открытие корпоративного task tracker в отдельном Chrome. Это ручная положительная проверка; ошибка браузера при выключенном gateway пока не проверена. После интерактивной HTTPS-авторизации Git прошёл read-only precheck и начал полное клонирование на NTFS через repository-local SOCKS; завершение клонирования пока не подтверждено. Эти результаты не переносят авторизацию на другого пользователя и не заменяют его собственную приёмку. Не прерывайте активные Git-передачи ради обновления forwarder или negative test.

Ранние Python-проверки сохранности Windows DNS/routes охватывали IPv4. Теперь CLI требует и успешное сравнение IPv6 DNS/routes; старый baseline без IPv6 не считается достаточным. Реальное чтение IPv6 проверено, но парная проверка при остановке/запуске VPN ещё предстоит. Старые снимки не переписываются ради успешного результата; новый baseline формируется только очередным контролируемым preflight.
