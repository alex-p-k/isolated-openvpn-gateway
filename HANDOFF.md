...
Последующий запуск Docker Desktop завершился ошибкой его Ingest service до появления Linux engine: Windows 1920 при доступе к устаревшему AF_UNIX socket. Отдельно разрешённое переименование только этого socket в backup не удалось ни через PowerShell, ни через штатный Win32 rename. Исходный объект не изменён, backup не создан; сброс Docker, удаление данных, изменение ACL или перезагрузка WSL не выполнялись. Работающий UDP-шлюз WSL и Windows IPv4/IPv6 DNS/routes, внешний public IPv4 egress и глобальные Git-настройки сохранены. Реальная Docker-приёмка остаётся заблокированной. Подробные границы диагностики — в `WINDOWS.md`; сырые Docker logs не передавать автоматически. Откат этой неудавшейся операции не требуется.
